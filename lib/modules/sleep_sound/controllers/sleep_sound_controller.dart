...
class SleepSoundController extends GetxController {
  ...
  //     // Only preload if we don't already have a player for this ID
  //     if (!soundPlayers.containsKey(sound.id)) {
  //       final player = AudioPlayer();
  //       soundPlayers[sound.id] = player;
  //
  //       // 🔥 IMPORTANT: Use catchError and DO NOT 'await' the setUrl
  //       // We want this to run in the background.
  //       player.setUrl(sound.file, preload: true).catchError((e) {
  //         debugPrint("Silent preload failed for ${sound.name}: $e");
  //       });
  //
  //       // Keep it ready for mixing
  ...
  //
  //   // 1. SILENT LOCK: Exit if already fetching this specific tab
  //   if (loadingKeys.contains(key)) return;
  //   if (!soundsBySubCategory.containsKey(key)) {
  //     await _hydrateFromCache(key);
  //   }
  //   // 2. CACHE CHECK: Do we have data?
  //   final bool hasData = soundsBySubCategory.containsKey(key) &&
  //       soundsBySubCategory[key]!.isNotEmpty;
  //
  //   // 3. SMART LOADER: Only show "Hard Loader" if screen is 100% empty
  //   if (!hasData) {
  //     loadingKeys.add(key);
  //   }
  //
  //   try {
  //     // --- SPECIAL CASE: YOUR MIXES ---
  //     if (cat == "mixes" && sub == "your-mixes") {
  //       await fetchMixes();
  //       soundsBySubCategory[key] = []; // Marker for the Mix UI
  //       return;
  //     }
  //
  //     // --- SPECIAL CASE: THE "ALL" TAB (Local Merging) ---
  //     // Your logs show the API fails on "__all__", so we merge locally
  //     if (sub == allSubSlug) {
  ...
  //     }
  //
  //     // --- STANDARD API FETCHING (Pagination) ---
  ...
  //
  //       // Check for 'records' key in Map response
  //       if (data is Map<String, dynamic> && data.containsKey('records')) {
  //         final List records = data['records'] ?? [];
  ...
  //         currentPage++;
  //       } else {
  //         // If data is just an empty list [] or null, stop the loop
  //         break;
  //       }
  //     } while (currentPage <= totalPages);
  //
  //     // 4. ATOMIC UPDATE: Replace data and refresh UI instantly
  ...
  //         // Preload only the top 3 (to save memory/data on large files)
  //         _preloadPageSounds(allItems.take(3).toList());
  //       } else {
  //         // Preload everything (small ambient sound files)
  //         _preloadPageSounds(allItems);
  //       }
  //     }
  //   } catch (e) {
  //     debugPrint("❌ Silent Fetch Error [$key]: $e");
  //   } finally {
  //     // 5. RELEASE LOCK: Always hide the loader
  ...
  //   final key = soundKey(categorySlug, subSlug);
  //
  //   // 1. SILENT LOCK: Prevent duplicate concurrent API calls for the same tab
  //   if (loadingKeys.contains(key)) return;
  //
  //   // 2. HYDRATE: Load from SharedPreferences first so user sees data in 0ms
  //   if (!soundsBySubCategory.containsKey(key)) {
  //     await _hydrateFromCache(key);
  //   }
  //
  //   // 3. SMART LOADER: Only add to 'loadingKeys' if the cache is actually empty.
  //   // This prevents the full-screen loader from showing during a background refresh.
  ...
  //       );
  //
  //       // Verify response success and data presence
  //       if (response != null && response['success'] == true) {
  //         final data = response['data'];
  //
  //         // Safety check: Backend sometimes returns empty List [] when no data,
  //         // or a Map {} for paginated records.
  //         if (data is Map<String, dynamic> && data.containsKey('records')) {
  //           final List records = data['records'] ?? [];
  //           allItems.addAll(records.map((e) => SoundItem.fromJson(e)).toList());
  //
  //           // 🔥 FIX: Update totalPages to fetch items beyond the first 10
  //           totalPages = data['total_pages'] ?? 1;
  //           currentPage++;
  //         } else {
  //           break; // Stop if data is not in expected Map format
  //         }
  //       } else {
  ...
  //
  //       // Overwrite cache with fresh data (updates Premium/Lock status)
  //       soundsBySubCategory[key] = allItems;
  //       _saveToCache(key, allItems);
  //
  //       // Notify Obx listeners to refresh the GridView
  //       soundsBySubCategory.refresh();
  //
  //       // Preload assets for a premium feel
  //       _preloadPageImages(allItems);
  //       if (cat == "music" || cat == "story") {
  //         _preloadPageSounds(allItems.take(3).toList());
  //       } else {
  ...
  //   } finally {
  //     // 6. RELEASE LOCK: Always clear the loading state
  //     loadingKeys.remove(key);
  //   }
  // }
...
// Private helper for background syncing
...
//
//       // 🔥 FIX: If the slug is "__all__", the API will fail.
//       // We must fetch each sub-category individually and merge them.
//       if (sub == allSubSlug) {
//         // final actualFilters = subCategoryMap[cat] ?? [];
//
//         // for (var filter in actualFilters) {
//         //   if (filter.slug == allSubSlug) continue; // Skip itself
//         //
//         //   final response = await SoundsApis.fetchSoundsRaw(
//         //     categorySlug: cat,
...
//         //   }
//         // }
//         final actualFilters = subCategoryMap[cat]!.where((f) => f.slug != allSubSlug);
//
//         final results = await Future.wait(actualFilters.map((f) =>
//             SoundsApis.fetchSoundsRaw(categorySlug: cat, subCategorySlug: f.slug)
//         ));
//
//         for (var response in results) {
//           final data = response['data'];
//           if (data != null && data['records'] != null) {
//             mergedItems.addAll((data['records'] as List).map((e) => SoundItem.fromJson(e)));
//           }
//         }
//       } else {
//         // --- Standard Single Sub-category Logic ---
//         final response = await SoundsApis.fetchSoundsRaw(
//           categorySlug: cat,
//           subCategorySlug: sub,
//           page: 1,
...
//       List<SoundItem> mergedItems = [];
//
//       // 1. Get the subcategories to fetch
//       List<SoundSubCategory> targetFilters = [];
//
//       if (sub == allSubSlug) {
//         // ✅ FIX: Use .value and cast to Map to avoid the String vs Int index error
...
//
//       // 2. Fetch EVERY page for EVERY subcategory
...
//               }
//             } else {
//               break; // Data is likely an empty list [] from API
//             }
//           } else {
//             break;
//           }
...
//       // 3. Final Update
//       if (mergedItems.isNotEmpty) {
//         // Deduplicate items to ensure the same sound doesn't appear twice in "All"
//         final ids = <int>{};
...
//       for (var filter in targetFilters) {
//
//         // 🔥 FIX 1: Har filter ke beech 300ms ka delay dein taaki 502 na aaye
//         await Future.delayed(const Duration(milliseconds: 300));
//
//         int currentPage = 1;
//         int totalPages = 1;
...
//           );
//
//           // 🔥 FIX 2: 502 error handling
//           if (response == null || response['success'] != true) {
//             debugPrint("⚠️ Server issue (502) on filter: ${filter.slug}. Skipping...");
//             break; // Loop break karke next filter pe jao
//           }
//
//           final data = response['data'];
//           if (data is Map<String, dynamic>) {
...
//           currentPage++;
//
//           // 🔥 OPTIONAL: Agar pages bahut zyada hain toh page ke beech bhi delay dein
...
// Baaki sab ko dispose karke memory free karein
  ...
  //   try {
  //     // Tell the player the URL (Network task)
  //     await musicPlayer.setUrl(music.file);
  //
  //     // Verify user is still on this song
  ...
  // void _loadAndPlayMusic(SoundItem music) async {
  //   try {
  //     // 🔥 THE FIX: Make sure it's a full network URL before giving it to the player
  //     String audioUrl = music.file;
  //     if (!audioUrl.startsWith('http')) {
  //       // Replace with your actual BASE_URL variable if you have one globally
  //       audioUrl = "https://api.sleepable.ai$audioUrl";
  //     }
  //     updateSimpleNotification();
  //     // Tell the player the full network URL
  //     await musicPlayer.setUrl(audioUrl);
  //
  //     // Verify user is still on this song
  //     if (playingMusic.any((m) => m.id == music.id)) {
  //       double initialVolume = playingSounds.isEmpty ? 1.0 : 0.5;
  ...
  //     }
  //
  //     // 1. Music player ko isolated rakhein (No system buttons)
  //     await musicPlayer.setUrl(audioUrl);
  //
  //     // 2. Dono Notifications update karein
  //     _updateSimpleTextNotification(); // Notification 1 (Tracking)
  //     _updateAudioNotification();       // Notification 2 (Audio Name)
  //
  //     if (playingMusic.any((m) => m.id == music.id)) {
  //       double initialVolume = playingSounds.isEmpty ? 1.0 : 0.5;
  //       await musicPlayer.setVolume(initialVolume);
  ...
  //   musicPlayer.positionStream.listen((pos) => currentPosition.value = pos);
  ...
  //     // 🔥 CRITICAL FIX: Ignore hardware updates while buffering or toggling
  //     // This prevents the UI from flipping back to "Paused" while loading
  //     if (_isToggling ||
  ...
  //     }
  //
  //     if (isTrackingActive.value && !state.playing && !isPauseAction.value) {
  //       musicPlayer.play();
  //       return;
  //     }
  //
  ...
  //     }
  //
  //     // 🔄 Sync UI only when hardware is definitely ready or idle
  ...
  // @override
  // void onReady() async {
  //   print("----- Configuring Audio Session for Mixing -----");
  //   super.onReady();
  //
  //   try {
  //     final session = await AudioSession.instance;
  //     // await session.configure(AudioSessionConfiguration(
  //     //   // category playAndRecord is essential for apps that do both
  ...
  //     //   ),
  //     //   // 🟢 Use gainTransientMayDuck: This tells the OS
  //     //   // "I am playing music, but I don't mind sharing the audio focus."
  //     //   androidAudioFocusGainType: AndroidAudioFocusGainType.gainTransientMayDuck,
  //     //   androidWillPauseWhenDucked: false,
  //     // ));
  //     // Add this inside your AudioSession configuration
  //     await session.configure(AudioSessionConfiguration(
  //       avAudioSessionCategory: AVAudioSessionCategory.playAndRecord,
...
// Separate helper to handle the "heavy lifting" without blocking the UI thread
//   void _startSoundBackground(SoundItem sound) async {
//     final id = sound.id;
//
//     try {
//       // 1. Get player (Check map first so we don't hit createPlayerForSound if already preloaded)
//       AudioPlayer player;
//       if (soundPlayers.containsKey(id)) {
//         player = soundPlayers[id]!;
//       } else {
//         player = await createPlayerForSound(sound);
//       }
//
//       // 2. Set settings
...
//       await player.setLoopMode(LoopMode.one);
//
//       // 3. Play (Only if the user hasn't already toggled it OFF while we were loading)
//       if (playingSounds.any((s) => s.id == id)) {
//         // seek(zero) is often what causes the 500ms lag on Android.
//         // Only seek if the player was previously at the end.
//         player.play();
//         syncVolumesAndLoop();
//       }
//       updateSimpleNotification();
...
// 🔥 Change 1: Ensure _isToggling is true during the transition to block listeners
  ...
  // }) async {
  //   debugPrint("🛑 [fadeOutAndStopAll] Called. Reason: $reason");
  //
  //   if (_isClearing) return;
  //   _isClearing = true;
  //   _isToggling = true;
  //
  //   try {
  //     // 🔥 FIX 1: Use .toList() to create a static snapshot.
  ...
  //       for (var p in activePlayers) p: p.volume
  //     };
  //
  //     // 📉 Fade Out Logic
  //     if (!isPaused.value) {
  ...
  //         await Future.wait(
  //           activePlayers.map((p) => p.setVolume((startVolumes[p] ?? 0.5) * fadePercentage)),
  //         );
  ...
  //
  //     // ⏹️ Stop all players
  //     await Future.wait(activePlayers.map((p) => p.stop()));
  //
  //     // 🔥 FIX 2: Reset main music player volume to 1.0 (or original)
  //     // so it's ready for the next use.
  //     await musicPlayer.setVolume(1.0);
  //
  //     // 🔥 FIX 3: Dispose only the transient sound players
  ...
  //
  //   } catch (e) {
  //     debugPrint("❌ [fadeOutAndStopAll] Error: $e");
  //   } finally {
  //     _isToggling = false;
  //     _isClearing = false;
  //   }
  ...
  //
  //   try {
  //     // 1. Static snapshot for active players
  //     final List<AudioPlayer> soundPlayersSnapshot = soundPlayers.values.toList();
  //     final List<AudioPlayer> activePlayers = [
  //       ...soundPlayersSnapshot,
  //       if (musicPlayer.playing) musicPlayer
  //     ];
  //
  //     // 🔥 FIX: Audio stop hote hi Notification ID 888 ko cancel karein
  //     // Isse lock screen se "Now Playing" wala text turant hat jayega
  //     await FlutterLocalNotificationsPlugin().cancel(id:888);
  //
  //     if (activePlayers.isEmpty) {
  //       _clearState();
  ...
  //     };
  //
  //     // 📉 Fade Out Logic (Music dheere dheere band hoga)
  //     if (!isPaused.value) {
  //       const int steps = 20;
  //       final int stepDelay = duration.inMilliseconds ~/ steps;
  //
  ...
  Future<void> restoreMixFromApi(MixedSoundRecord mix) async {
    try {
      ...
      for (var item in mix.sounds) {
        ...
        //   image: item.image,
        //   file: item.file,
        //   categoryName: item.categoryName ?? 'Mix',
        //   // ... fill other required SoundItem fields as you did before
        // );
  ...
  //   String notifyContent = "Sleep tracking is active";
  //
  //   if (playingMusic.isNotEmpty) {
  //     notifyContent = "${playingMusic.first.categoryName} Playing: ${playingMusic.first.name}";
  //   } else if (playingSounds.isNotEmpty) {
  //     notifyContent = "Ambient Sounds Playing";
  //   }
  //
  //   // Check karein agar service chal rahi hai, tabhi update karein
  //   if (await FlutterForegroundTask.isRunningService) {
  //     FlutterForegroundTask.updateService(
  //       notificationTitle: "Sleepable AI",
  //       notificationText: notifyContent,
  //     );
  //   }
  // }
  Future<void> _updateSimpleTextNotification() async {
  ...
  // SleepSoundController.dart ke andar ye method update karein

  // Future<void> _updateAudioNotification() async {
  //   final FlutterLocalNotificationsPlugin localNotifications = FlutterLocalNotificationsPlugin();
  //
  //   // 1. Agar kuch bhi play nahi ho raha toh 2nd notification hata dein
  //   if (playingMusic.isEmpty && playingSounds.isEmpty) {
  //     await localNotifications.cancel(id: 888);
  //     return;
  //   }
  //
  //   // 2. Text decide karein
  //   String notifyContent = "";
  //   if (playingMusic.isNotEmpty) {
  //     final item = playingMusic.first;
  //     notifyContent = "${item.categoryName} Playing: ${item.name}";
  //   } else if (playingSounds.isNotEmpty) {
  //     notifyContent = "Ambient Sounds Playing";
  //   }
  //
  //   // 3. Android settings (Simple, No buttons, No sound)
  //   const AndroidNotificationDetails androidPlatformChannelSpecifics =
  //   AndroidNotificationDetails(
  //     'audio_status_channel',
  //     'Audio Status',
  ...
  //
  //   // 4. 🔥 Correct Named Parameters implementation
