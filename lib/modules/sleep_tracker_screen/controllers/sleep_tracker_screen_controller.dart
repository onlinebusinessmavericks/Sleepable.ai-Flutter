...
class SleepTrackerController extends GetxController with WidgetsBindingObserver {
  ...
  RxDouble dimAmount = 0.0.obs;
  Timer? _dimTimer;
  DateTime? _lastSavedTimeProcess;
  ...
  /// 🔥 EXTERNAL TRIGGER POINT (Called from BPM screen button click)
  Future<void> triggerInstantBackgroundService() async {
    toast("🔍 Step 1: Checking Permissions...");
    ...
    toast("🛠️ Step 2: Initializing Native Task channels...");
    _initService();

    toast("📡 Step 3: Launching Foreground Service...");
    await _startNoiseMeter();
  }
  ...
  Future<void> _initController() async {
    ...
    _startMotionTracking(); // 🟢 Start Accelerometer
    _startNightMapping();   // 🟢 Start Data Logger
    _startUiDbTimer();
  ...
  Future<void> _onNoise(NoiseReading reading) async {
    ...
    final now = DateTime.now();
    if (_lastSavedTimeProcess != null && now.difference(_lastSavedTimeProcess!).inMilliseconds < 1000) return;
    _lastSavedTimeProcess = now;
  ...
  void _listenToAmplitude() {
    ...
    _amplitudeTimer = Timer.periodic(const Duration(milliseconds: 200), (timer) async {
      ...
      try {
        final amp = await recorder.getAmplitude();
        if (trackerState.value == TrackerState.idle) return;
  ...
  void _startUiDbTimer() {
    _uiDbTimer?.cancel();
    _uiDbTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      ambientNoise.value = rawDb.value;
    });
  }
  ...
  Future<bool> stopSleepTracker(int sleepTrackerId) async {
    ...
    try {
      ...
      final response = await TrackerApis.stopSleepTracker(sleepTrackerId: sleepTrackerId);
  ...
  void _initService() {
    ...
    FlutterForegroundTask.init(
      androidNotificationOptions: AndroidNotificationOptions(
        channelId: 'sleep_tracker_channel',
        channelName: 'Sleepable AI Tracker',
        channelDescription: lang.serviceText,
  ...
  Future<void> _startNoiseMeter() async {
    if (_noiseStarted) {
      toast("ℹ️ Info: Service is already running.");
      return;
    }
    final lang = Get.context!.lang;
    ...
    if (!isRunning) {
      try {
        ...
        // 🔥 DEBUG TOAST 1: Native Service Result check karne ke liye
        toast("📊 Native Service Start: ${success.toString()}");
        ...
        if (Platform.isIOS) {
          ...
          _noiseStarted = true;
          // 🔥 DEBUG TOAST 2: Apple background lifecycle response trace karne ke liye
          toast("🍏 iOS Background Tray Notification Triggered!");
        } else {
          _noiseStarted = true;
        }
      } catch (e) {
        // 🔥 DEBUG TOAST 3: Kuch bhee native crash hua toh direct screen par aayega
        toast("❌ Native Exception starting service: ${e.toString()}");
      }
    } else {
      _noiseStarted = true;
      toast("✅ Service state confirmed: Already running active worker.");
    }
    ...
    if (_noiseStarted) {
      ...
      // 🔥 DEBUG TOAST 4: Stream binding success trace karne ke liye
      toast("🎙️ Audio Streams Attached & Listening!");
    } else {
      // 🔥 DEBUG TOAST 5: Agar task trigger block ho gaya ho
      toast("⚠️ Alert: Service started but task handles failed to initialize!");
    }
  }
  ...
  Future<void> _uploadInBackground(String path) async {
    ...
    try {
      ...
      final soundController = Get.find<SleepSoundController>();
      bool isAnythingPlayingInApp = soundController.playingSounds.isNotEmpty || soundController.playingMusic.isNotEmpty;
      bool isMusicLikely = isAnythingPlayingInApp || (_baselineDb > 40.0);
